...
| 8 | tier name "Air-Gap / Defence" (page card + `tiers.py` label) | names the *deployment*, not what the buyer pays for | see B-085 decision 3 |
...
3. **Rename the top tier** (raised 2026-09-24 by the user). "Air-Gap / Defence" names the *deployment*, not what the buyer pays for — contract, SLA, onboarding, in-network licence server. Name the buyer instead (candidate: **Defence / Regulated**, or **Enterprise**), and keep "air-gap" as a feature line inside the tier, not the tier name. The doc already half-hedges ("Air-Gap / Defence"); the second word is the real one.
   **Caveat — do NOT rename the tier *key*.** `tier` is inside the signed licence payload (`src/licensing/license.py:16`, `:101`) and `license_status` falls back to `free` for any unknown tier (`license.py:264`). So changing the key `airgap` would silently downgrade every Air-Gap licence already issued. Change the **label** only ("Air-Gap Premium" → the new name) and keep the wire value `airgap`; an alias map is the alternative if the key must change. Also update the Lemon Squeezy placeholder slug on the page (`TANCAT-AIRGAP-TBD`).
...
## 🆕 B-092 — Remaining false greens: criteria that name content or a specific element still pass on a bare `assert_visible`

**Status:** ✅ **Fixed 2026-09-24** — branch `fix/b090-b092-page-facts` (session record `docs/sessions/2026-09-24_session9_b090_b092_page_facts.md`). The content/wrong-element half of the gate-2 audit, read from the emitted test file (`scratch/session7_evidence/emitted_test_file.py`). Found while closing B-088 (2026-09-24).
**Priority:** **high** — this is the remaining half of **gate 2 ("zero false greens")** after B-086/B-087/B-088 closed the head/page/section/attribute classes. **B-090** is the geometric half (broken images, natural width, horizontal scroll, card count); this is the content/wrong-element half.
**One-line:** a criterion that names text or a specific element still emits `assert_visible(<nearest element>)`, which passes without checking the named thing:

| Criterion | Emitted | What it really checked |
|---|---|---|
| 01 — hero headline "The AI test generator …" visible on load | `assert_visible('p.leading-relaxed…text-sm')` | a **paragraph** is visible, not the `<h1>` that carries the headline text. |
| 03 — "shows at least four capability cards" | `assert_visible(<hero paragraph>)` | a paragraph (B-090 owns the count half). |
| 04 — section shows install command `git clone …` | `assert_visible('.text-white')` | an element with that class; the command string is never checked. |
| 08 — Noir Art image loads rather than a blank panel | click + `assert_visible('#btn-tab-mascot')` | the **button** — the image was never inspected. |
| 11 — "Pro Deployment tier shown with a price" | `assert_visible(h3)` | the heading only; no price (12 checks Air-Gap's price, so the pattern exists). |
| 18 / 19 — every header-nav / same-page anchor link points at an existing element | `assert_attribute(<one anchor>, 'href')` | **one** link, not "every". |

**Expected behaviour:** a content criterion must read the text (`assert_text_contains` / a scoped text check), and a "specific element" criterion must resolve to that element rather than a same-section neighbour — the same shape as B-088's 16/22 but for content, not links. A "every X" criterion is a page-level scan (`assert_attribute_all`, or an anchor-target-exists scan for 18/19). Where a real check is impossible, skip honestly.
**Where:** `src/code_postprocessor.py` classifiers + `src/evidence_tracker.py` (content/scan checks); the wrong-element half is `src/element_matcher.py` / `src/placeholder_scorers.py`.
**Note:** the Session 8 re-run's emitted file was not preserved; the Session 7 file is the evidence for the shapes above (the re-run confirmed 31 passed, i.e. these checks still pass). A cheap follow-up is to have the re-run harness copy the emitted test file next to the suite log — then this audit is mechanical. **Done:** the harness now copies the emitted file to `scratch/session8_emitted_test_file.py`.
**Fix:** content/scan families at the emit chokepoint: an install command → `assert_text_contains('body', <command>)` (c4); a literal price → `assert_text_contains('body', <price>)` (c10); "<tier> price" → `assert_section_has_price(<tier>)` (c11/c12); "every same-page anchor link …" / "anchor links resolve" → `assert_anchor_targets_exist()` (c18/c19). Wrong-element half: `src/content_scoping.py` scopes "headline" to `h1` (c1 → the real `<h1>`) and image criteria to `<img>` (c8 → the artwork image), with a post-resolution `kind_matches` guard; `src/placeholder_scorers.py` `_kind_bonus` ranks the named image first (alt-token overlap + visible-over-hidden + logo penalty) and waives the hidden-element penalty for image criteria; `src/semantic_candidate_ranker.py` now includes `alt` in the candidate prompt; the scraper extracts `<img alt>`. `src/journey_scraper.py` runs its scrape subprocess with the checkout root on `PYTHONPATH` so the child imports the branch's `src`, not the main repo's editable install.
**Verification:** same live re-run — c1 `assert_visible(h1…)`, c4 `assert_text_contains('body', 'git clone …')`, c11 `assert_section_has_price('Pro Deployment')`, c18/c19 `assert_anchor_targets_exist()`; c8 `assert_natural_width('img…object-cover…')` (the artwork image). Zero false greens.
**Estimated sessions:** 1–1.5.
...
**Status:** ✅ **Fixed 2026-09-24** — branch `fix/b090-b092-page-facts` (session record `docs/sessions/2026-09-24_session9_b090_b092_page_facts.md`). Found while closing B-088, same Session 7 emitted test file (2026-09-24).
...
**Fix:** new `PageFactAssertion` classifier + `_emit_page_fact_assertion` in `src/code_postprocessor.py` emit the page-fact family before element resolution: `assert_no_broken_images()` (c7), `assert_natural_width(selector)` (c6/c8), `assert_count_at_least(selector, n)` (c3), `assert_no_horizontal_scroll(width=375)` (c35). New tracker methods in `src/evidence_tracker.py`. `_is_page_level_assert` treats the no-resolution families as page-level so an unresolved resolver result does not insert a journey skip. The scraper now extracts `<img>` elements (with `alt`) — without them an image criterion had no candidate; `src/content_scoping.py` scopes image criteria and the scorer's `_kind_bonus` + image visibility waiver lift the named image above the threshold.
**Verification:** live re-run of the 35-criterion story — c3 `assert_count_at_least('[class*="card"]:has(h3), [class*="tile"]', 4)`, c6 `assert_natural_width('img.h-full.object-contain.w-full')`, c7 `assert_no_broken_images()`, c35 `assert_no_horizontal_scroll(width=375)`. Result **2 failed / 31 passed / 2 skipped**; the artwork was re-hosted locally (`landing/noir_art.jpg`) so c7/c8 now pass for real, the 2 reds are the stale c13/c14, the 2 skips are c16/c17. Zero false greens. The price classifier rejects generic content names ("product name and price") so a cart-content criterion cannot emit a bogus section check. 62 tests in `tests/test_b090_b092_page_facts.py`.
**Estimated sessions:** 1–2.
...
- [x] **B — assert the description is actually reflected:** for href/attribute assertions ("no TBD in hrefs", "href is a live video") the emitted check must read the attribute, not assert visibility of a container. **Done 2026-09-19** — the 2026-09-18 skeleton (commit 42ab0bd: `attribute_assertion_type` + `assert_attribute`) read the attribute but only checked **non-emptiness**, so `href="https://lemonsqueezy.com/l/TANCAT-PRO-TBD"` still passed a "does not contain TBD" check. This session completes it: `attribute_predicate()` (`src/code_postprocessor.py`) derives the predicate from the description — explicit forbidden words ("does not contain TBD" → `forbidden=('tbd',)`; "does not contain YOUR_VIDEO_ID_HERE" → `('your_video_id_here',)`) and, when the value must be live ("live"/"valid"/"resolv"), the placeholder vocabulary + `must_be_url` for hrefs. `EvidenceTracker.assert_attribute` enforces it (shared `_attribute_violations`). Assert-kind classification also gained the **count kind**: `count_assertion_from_description()` maps "no TBD in links" → `assert_no_forbidden('a', 'tbd', attribute='href')` (links→href, buttons/headings→text), "no lorem ipsum in headings" → text scan, "all images have alt" / "every image has a non-empty alt" → `assert_attribute_all('img', 'alt')`, "all anchor links valid" → `assert_attribute_all('a', 'href', forbidden=<placeholders>)`. The intercept sits in the single emit chokepoint (`replace_token_in_line` — all three resolution paths flow through it) **before** the part-(a) unverified skip, so a page-level check runs even when the resolver could not verify an element. New tracker methods: `assert_no_forbidden` (zero matches passes vacuously, count recorded for the evidence) and `assert_attribute_all` (zero matches FAILS — the condition expects the elements). 48 tests in `tests/test_b069_false_greens.py` (classification, predicates, emit, enforcement).
  **Live verification** — the exact emitted calls replayed against the real landing page (`scratch/verify_b069b_live.py`, gitignored): **10/10 honest.** The 09-17 false greens now FAIL: `TANCAT-PRO-TBD` / `TANCAT-AIRGAP-TBD` (`forbidden ('tbd',)`), `YOUR_VIDEO_ID_HERE` (`must_be_url` + placeholder vocabulary), "no TBD in links" → *"Found 2 element(s) matching a containing 'tbd'"*, "all anchor links valid" → *"4/24 fail: contains forbidden 'your_'"*; the genuine checks pass with recorded counts (3 buttons, headings, 4 image alts, hero `h1`). Resolve-level wiring verified on the live scrape: the Buy Pro criterion resolves to the real link (score 13, text match, **not** a fallback) with `assertion_type=toHaveAttribute:href`.
  **Pending (not waived):** the plan's "0 false greens on the same run" — the full-LLM regeneration of the 35-criterion story. Two consecutive `ci_generate.py` attempts stalled in the skeleton LLM call (600s × 2, then clean exit; `scratch/b069b_ci_generate.log`) — the 27B model-state class from `docs/sessions/2026-09-16_b065_rerun_llm_stall.md`, not a code defect (server `/health` ok, resolve/scrape fine). The 14-criterion re-run story covering every classification branch is saved at `scratch/b069b_story.md` for a healthy-model re-run: `uv run python scripts/ci_generate.py --story scratch/b069b_story.md --url http://localhost:8079/ --danger-zone --workspace b069b-verify`.
- [ ] **C — surface it:** count "assertions that matched a generic container" in the report (AI-067 badge gives the pattern) so a human can spot-check. (Lower value now that A+B emit honest skips/reds; keep as a report-nice-to-have.)
- Recommendation: **A + B**. A stops the false greens immediately; B fixes the underlying assertion-kind gap. **Both done 2026-09-19.**
**Estimated sessions:** 1–1.5 (spent: 1).
...
**Status:** ✅ **Fixed 2026-09-21** (pending commit). Both options implemented:
- **(A) follow the new page:** `_verify_click_navigation` now polls for a new tab (2.5s window, then a 7.5s window after the overlay-dismissal retry — headless tab creation was measured at up to ~8s late on this machine's Chromium 151; probe scripts in `scratch/probe_delay*.py`). When found: wait for `domcontentloaded` (8s cap), compare the tab's URL with the element's href (netloc + path), record `element["new_tab"] = {url, matched_href}` on the click step, close the tab, treat the click as passed. `target` is now captured in `_get_element_metadata`. When NO tab appears, the failure message names the cause (headless Chromium drops new-tab anchor clicks) and recommends the href check — instead of the old overlay-swallow guess. `navigate()` also closes stray tabs a previous step leaked, so one late tab cannot poison the next test.
- **(B) verify without navigating:** resolve/404 criteria no longer click. `attribute_assertion_type` now classifies "resolves" / "404 + link|href" descriptions as `toHaveAttribute:href` → the existing B-069(b) path emits `assert_attribute(..., must_be_url=True, forbidden=placeholders)` (reads the href; no click, no tab juggling). Both skeleton prompts (`build_skeleton_prompt` + `build_single_condition_prompt`, plus the byte-identical legacy templates in `prompt_utils.py`) gained a rule steering such criteria to `{{ASSERT:<link text> link resolves}}` instead of CLICK.
- **Scope note (honesty):** the emitted check validates a live-shaped http(s) URL without placeholders — it does not HTTP-probe the destination's status (option B's `context.request.get` was not needed for the failure class; a live-status probe is a possible follow-on, not part of this fix).
- **Evidence:** live replay `scratch/verify_b072_live.py` — 8/8: all four 09-17 reds (tc01_20..23: GitHub, Egress Audit, Security Policy, License) PASS via the href check; the `target="_blank"` GitHub click itself was detected, verified (`matched_href=True`), recorded and closed on the real landing page; TBD purchase links stay honest reds. +11 unit tests (`tests/test_evidence_tracker.py` new-tab/mismatch/no-tab/window.open/blank-message cases, `tests/test_b069_false_greens.py` classification + emit cases, `tests/test_prompt_builder.py` rule presence).
...
**Options**
- [x] **A — follow the new page:** implemented with a polling window over `context.pages` (a `pages_before` snapshot in `click()` + `_find_new_tab` diff) rather than the `context.on("page")` event — same information, no handler lifecycle. URL verified against the href; tab closed afterwards.
- [x] **B — verify without navigating:** implemented as the href ATTRIBUTE check (`assert_attribute(must_be_url=True, forbidden=placeholders)`) rather than an HTTP GET — it closes the failure class (false reds on unclickable new-tab links) without egress or status probing.
- Recommendation (as shipped): **B for resolve/404 criteria, A for true navigation criteria** — the criterion kind already existed in the assertion plumbing (B-069 option B).
**Estimated sessions:** 0.5.
...
**Status:** ✅ **Fixed 2026-09-21** — option A: `_extract_numbered_criteria` now tolerates isolated interruptions. A non-numbered, non-empty, non-bullet line (a group heading like `IMAGES`, an isolated sentence) no longer ends the list; only **3+ consecutive** such lines do (a genuinely different section). Blank lines and bullets are neutral (they neither interrupt nor extend the streak). Verified against the exact repro shape through the real UI path (`parse_requirements_text` → `analyze`): 35-criterion story with group headings → **35/35 (was 5)**, deterministic 1:1, zero LLM calls; a trailing 3-line prose section still ends the list and its numbered lines are not swallowed. +5 tests (`tests/test_spec_analyzer.py`) + e2e replay `scratch/verify_b063_e2e.py`.
...
**Options (pick one)**
- [x] **A — tolerate isolated interruptions (recommended):** keep scanning after a non-numbered line; break only after several consecutive non-numbered, non-empty lines (e.g. 3). Headings, blank lines and the odd sentence between criteria then survive, while a genuinely different section still ends the list. **Done 2026-09-21** (threshold = 3, implemented as a streak counter).
...
**Status:** ✅ **Fixed 2026-09-21** — options A + C, plus a deeper root cause the e2e replay uncovered (the parser produced line fragments, not the documented single wrapped item — see Fix below).
...
**Fix (2026-09-21)**
- **The root cause was deeper than the guard.** The e2e replay showed `parse_requirements_text` does NOT wrap a multi-line story as one item — `FeatureParser.build_requirement_model` treated **each wrapped line** as a separate requirement (`derived_from_story`), producing sentence-fragment "criteria" (`1. this is the landing page…images,` / `2. pricing and a demo…`), so the 1-item guard could never fire and the user got one test per *line fragment*. `src/user_story_parser.py` now joins a line that starts lowercase after a line that does not end a sentence (`_join_wrapped_lines`), so a wrapped paragraph stays one requirement and the wrap becomes the documented `1. <whole story>`; capital-started (sentence-shaped) lines still stay separate.
- **Option A — widened guard** (`src/spec_analyzer.py`): `_has_multi_concern_signal` also fires when the text is >200 chars, or carries a coordinating list (2+ commas) with 2+ test verbs (`check`, `verify`, `should`, `shows`, `links`, `can be`). The `analyze()` single-item gate also checks the **full spec text** — a wrapped blob extracts only its first line as the item, and that line may carry no signal alone. The wrapped story now reaches the LLM splitter.
- **Option C — surfaced** (`single_condition_warning` in `src/spec_analyzer.py`, rendered in `streamlit_app.py`): when exactly one condition is derived from a story >200 chars, the plan panel warns to list numbered acceptance criteria.
- **Verified:** e2e replay of the reported story (`scratch/verify_b062_e2e.py`): wrapped as a single item, routed to the LLM (mock split accepted), and the LLM-collapse case surfaces the warning. +11 tests across `tests/test_spec_analyzer.py` / `tests/test_user_story_parser.py` (one pinned line-fragment test re-pinned to the joined behaviour). Gates: 3295 pytest, smoke 39/39, ruff + mypy clean, eval static 97.9% (0.0pp drift).

**Options (pick one; do not stack them)**
- [x] **A — widen the guard**: also treat as multi-concern when the wrapped item is long (e.g. > ~200 chars) or contains coordinating lists (`and` / comma-enumeration) and multiple test verbs (`check`, `verify`, `should`, `shows`, `links`, `can be`). Cheapest, deterministic, no extra LLM call. **Done 2026-09-21.**
...
- [x] **C — surface it to the user**: when only one condition is derived from a story longer than N characters, show a warning ("we found one criterion — did you mean to list acceptance criteria?") and offer the numbered-criteria template. This does not fix the count but stops the silent surprise. **Done 2026-09-21** (N = 200, `single_condition_warning`).
...
**Priority:** Medium — it is a *diagnostic* fix for the class that keeps being misread as model failure (see B-060). It does not change resolution.
**One-line:** a step can run on a page other than the one its locator was resolved against, still pass, and report nothing. Now the generator emits `expected_page=` on each step, the tracker records `page_mismatch` on the step result, the report shows a **wrong page** badge, and the locator learner refuses to learn from such a step.
**Why the last part matters:** `src/rag_learn.py` learns golden patterns from passing steps. A step that ran on the wrong page may "work" only because a page-level container matched, so learning it would teach the store a bad pattern.

**What was built**
- `src/code_postprocessor.py` — `replace_token_in_line` gained `expected_page`; a wrapper appends `expected_page=<page>` to the emitted tracker call. **Only** `click` / `fill` / `assert_visible` / `assert_hidden` are annotated — those are the only tracker methods this pipeline emits (`toHaveText` is remapped to `assert_visible`), and emitting the keyword on a method that does not accept it would raise `TypeError` in the generated test. A test pins that guard.
- `src/placeholder_orchestrator.py` — passes `expected_page=current_url` at the emit site (the page the placeholder was scoped to).
- `src/evidence_tracker.py` — `click`/`fill`/`assert_visible`/`assert_hidden` accept `expected_page`; `_record_step` records `result["page_mismatch"] = {"expected", "actual"}`; `_same_page` compares scheme+host+path, ignoring query/fragment (SPA mocks encode state in the query) and failing open on unparseable input.
- `src/evidence_report.py` — amber **wrong page** badge on the step, with both URLs in the tooltip.
- `src/rag_learn.py` — a step carrying `page_mismatch` is never learned as a golden.

**Bug found and fixed during verification (important):** the first version compared the page **after** the action. Banked run: **11 flagged steps, every one a false alarm** — a click is *on* its expected page and then navigates away. Fixed by capturing the page **before** the action (`self._step_entry_url`), which is the page the step actually runs on. Re-run of the same generated tests: **2 flagged, 0 false alarms**, from 39 steps — and both remaining flags are genuine (resolver expected the login page for the "Transfer Money" click while the test is on the dashboard).
**Scope caveat:** the other tracker methods (`assert_text`, `assert_count`, `assert_disabled`, …) are not covered. They are unreachable from this pipeline today, so coverage is complete in practice; if one becomes reachable it must be added to `_EXPECTED_PAGE_METHODS` **and** given the parameter, or the annotation is silently skipped.
**Files:** `src/evidence_tracker.py`, `src/code_postprocessor.py`, `src/placeholder_orchestrator.py`, `src/evidence_report.py`, `src/rag_learn.py`, `tests/test_expected_page_annotation.py` (new), `tests/test_evidence_tracker.py`, `tests/test_rag_learn.py`, `tests/test_orchestrator.py`.
**Estimated sessions:** 0.5 (remaining: ship-it).
...
**Evidence:** banking `--regenerate --mode full --test-output scratch/mock_tests` → `Tests executed: 0` + 8 fixture errors (manual pytest, same files, same mock: 8 errors, `fixture 'evidence_tracker' not found`); `scratch/mock_tests_e2e` with the conftest copied in → `Tests executed: 8`.
**Estimated sessions:** 0.5.

---

## 🆕 B-060 — Mock page scoping: the AI-052 divergence latch fires on a MISSING trail observation, not a divergence

**Status:** ✅ **Fixed 2026-09-15** — Candidate A applied (the safety switch now needs a note that *contradicts* where the test is, not merely a missing note), Candidate B reverted. Measured across the three offline mocks: banking **4/13 → 7/13**, ecommerce unchanged (15/16), ambiguous **3/4 → 2/4** — overall **22/33 → 24/33**. The ambiguous loss is accepted as the honest result (it exposes a real trail mis-assignment the old behaviour was masking); that follow-up is owned by **B-055**. Regression test: `tests/test_resolver_observed_scope.py::test_missing_trail_note_does_not_latch_divergence` (red without the fix, green with it). Decision rationale (user, this session): *"although it was getting the right answer on the old behaviour it was doing it by accident and that's not what we're looking to do when it comes to testing. Getting the right answer accidentally is not a positive — that's not helping us prove that this works correctly for the user."* Gates: 3178 pytest, smoke 39/39, ruff + mypy clean, eval static 97.9%.
**Priority:** High — it silently dead-ends most steps on the mock sites, and it is the reason banking looked like an LLM/model failure for weeks.
**One-line:** a GOTO whose description `resolve_url` cannot match produces **no** `navigate` step, so that placeholder has no trail observation (`obs is None`); `src/placeholder_orchestrator.py:892` (`if obs is None or canon(obs.to_url) != landed: diverged = True`) reads that as divergence, latches the AI-052 divergence flag, and pins **every remaining step in the journey** to `last_verified_url` — the start page. Elements on the dashboard/transfer/payment pages are then never in the candidate pool and the steps honestly skip.

**Root cause (two halves)**
1. `src/orchestrator.py` `_scrape_one`: the `navigate` step was appended only `if url:` — an unmapped GOTO recorded **nothing**, even though the browser does start at the starting URL. AI-052 S4 then deleted the keyword-URL fallback, so an unmapped GOTO has no path at all. The codebase already treats this as normal: `tests/test_resolver_observed_scope.py::test_trail_map_matches_by_description_across_action_kinds` asserts *"GOTO 'products' produced no navigate step → unmatched"*.
2. The latch itself is **correct** and must not be weakened blindly — it guards a real false positive (the 2026-08-22 scraper/resolver disagreement: discovery clicked the add-to-cart button, the resolver picked the title link, the test navigated somewhere the trail never saw). Pinned by `test_divergence_latches_trail_no_longer_trusted`, and part of AI-052's ship evidence (zero `_LocatorNotFoundError: different page` on both verify sites).

**Evidence (banking, minimal repro, `PIPELINE_DEBUG=1`)**
```
ASSERT 'accounts dashboard loaded'
  obs=scrape|accounts dashboard loaded  from=dashboard.html   <- trail is CORRECT
  current=index.html                    last_verified=index.html  diverged=True
deferred-assert url=index.html scoped=True pages=['.../index.html']
```
The trail knew the right page; the latch discarded it. Result: the dashboard assert resolved against the login page and produced `main:has-text("<login page text>")`.

**Candidate A — latch only (MEASURED 2026-09-15):** drop the `obs is None` half (`if obs is not None and canon(obs.to_url) != landed`). Does not touch the trail, so it **cannot** move any other site's trail/scoping. `_trail_step_scope_url(None, …)` already returns our own `last_verified_url`, so a missing note cannot leak a locator to another page. No test pinned the `obs is None` half. Cost: if there is no note *and* the test really is off route, we keep trusting the trail's later notes.

**MEASURED — all three candidates, same model, RAG on, temp 0, `--mode static --regenerate`, 2026-09-15:**

| Site | no change (latch as-is) | **Candidate A** (latch only) | Candidate B (record navigate) |
|---|---|---|---|
| eval-007 banking | 4/13 (31%) | **7/13 (54%)** | 7/13 (54%) |
| eval-006 ecommerce | 15/16 (94%) | 15/16 (94%) | 15/16 (94%) |
| eval-010 ambiguous | **3/4 (75%)** | 2/4 (50%) | 2/4 (50%) |
| **overall** | 22/33 (67%) | **24/33 (73%)** | 24/33 (73%) |

**My earlier recommendation was wrong. A is NOT a free win — it costs exactly what B costs.** Corrected finding: **the ambiguous mock's 3/4 DEPENDS on the buggy latch.** `#quantity` exists only on `index.html`. With the latch on, that step is pinned to `last_verified_url` = `index.html` and resolves. With A *or* B the latch stays off, the step is scoped by the trail's `from_url` = `success.html`, and it fails — `Failed to find 'quantity' (current=.../success.html?item=blue-top&qty=1)`, 3 occurrences. This is a **clean controlled comparison**: A and "no change" produce byte-identical trails (`[success.html?item=blue-top&qty=1]` / `[success.html… -> index.html]`), so the only variable was the latch. The earlier claim that B's ambiguous loss was caused by B changing the trail was **also wrong** — B's failure is the same failure, from the same trail `from_url`.
**Net position:** both fixes trade 1 ambiguous placeholder for 3 banking placeholders (+2 overall). The real defect behind the ambiguous loss is the trail itself — the scraper's "select item" click records a landing on `success.html`, so the order-form step is assigned the success page. That is the page-context/trail-assignment class already owned by **B-055**, not the latch.

**Candidate B — record the navigation (REVERTED 2026-09-15):** always append the `navigate` step, using `url or starting_url`. Banking also went from 2 skipped placeholders to 1, and its regenerated suite from `1 passed / 1 failed / 6 skipped` to `3 passed / 4 failed / 1 skipped`. **Second, unmeasured flaw:** it applies to EVERY GOTO, so a mid-journey unmapped GOTO would make the scraper navigate back to the starting URL. **Reverted** in favour of A, which reaches the same numbers with no such flaw and no change to the scraper's behaviour.

**Decision — KEEP A (made 2026-09-15):** A stays. It reaches the same numbers as B with no change to the scraper's behaviour, and the eval-010 loss is the honest outcome rather than a regression to hide: the old latch was masking a wrong page assignment by searching the wrong page for the right reason. The order-form note is then fixed under **B-055**. Do not resurrect B. Remaining work on this item: commit + ship (it is currently uncommitted).
**Workaround while open:** the AI-067 `expected_page` / `page_mismatch` marker makes a start-page-scoped step visible in the report.
**Regression test:** `tests/test_resolver_observed_scope.py::test_missing_trail_note_does_not_latch_divergence` pins A (verified red without it — the assert is declared unresolved and a consolidated skip is inserted — green with it). The candidate-B test was deleted with the revert: `test_unmapped_goto_still_records_navigate_step`.
**Session record:** the 2026-09-15 session (banking mock scoping trace).
**Estimated sessions:** 0.5 (revert B, apply A, re-run the 3-mock A/B + eval gate).
...
**Mechanism pinned (2026-09-20, during the B-068/B-070 ship-it — status unchanged, original 09-07 repro not re-verified):** the pre-commit hook does NOT check the working tree — `check_mode` runs against a checkout of the **commit being constructed** (pre-commit stashes everything that differs from HEAD, including staged changes: the stash patch files in `~/.cache/pre-commit/` prove it). `check_mode` regenerates the board from that checkout's `BACKLOG.md` + `ROADMAP_ROADTO_PRODUCTION.md` and compares against that checkout's `kanban.html`. So the hook fails exactly when **the `kanban.html` in the commit-under-construction was generated from a different source state than that commit's tree** — e.g. a roadmap-only commit while the working-tree kanban was generated with the new BACKLOG (reproduced twice on 2026-09-20; manual `--check` on the full working tree passed while every commit attempt failed). Workaround that works: commit each source state together with the kanban generated *for that state* (roadmap commit ⇒ kanban generated from new-roadmap + old-BACKLOG; BACKLOG commit ⇒ kanban generated from new-BACKLOG + new-roadmap).
**Session record:** the AI-039 ship-it (this session); mechanism note 2026-09-20.
...
> **Follow-on open:** the divergence latch added here misfires on a *missing* trail observation — see **B-060** (status lives there, not here).
