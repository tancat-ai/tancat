...
- [ ] **Record the baseline** — document the expected gate set + the expected-unresolved placeholder names, then wire `verify_production` into CI with an expected-failure allowlist so a *new* failure becomes visible while the known 2 stay tolerated.
...
**Status:** 🆕 new — opened 2026-09-11, surfaced while removing the stray `nul` file that had been crashing mypy outright (see CHANGELOG [Unreleased] Fixed).
