...
**Next: B-093 — the held-out gate re-measure FAILED (66.4% resolution, 16 false greens).**
Session 10 ran the 9-story golden set live (`eval_harness run --regenerate --mode full`): the landing-page
numbers did **not** generalize. The deficit is concentrated in the stateful / auth-gated stories
(36 of 38 misses) — the B-054 / B-055 / AI-064 capture-and-scoping family, not the landing-page classes.
**Next: re-run the held-out set to re-score gate 1, then B-096.** The B-054 pool defect is fixed
(hidden SPA later-step fields now stay candidates on a decisive text match; `_is_fillable` role sets
aligned) — lv_insurance live **9/24 → 16/24**. The remaining lv misses are pass-1 first-match and
`main*` vs `addDriver*` ambiguity (**B-096**). Route the harness through the worktree with
`PYTHONPATH=<worktree root>` (**B-095**) before trusting any worktree measurement.
Records: `docs/sessions/2026-09-24_session10_heldout_gates.md`, `..._session11_b054_spa_pool.md`.
