...
**Next: gate 2 — the 16 false greens (B-093) — then the remaining gate-1 misses.** Session 12
(2026-09-25) fixed B-095 (worktree measurement integrity), B-096 (Pass-1 twins / required-marker
asymmetry), and B-094 (per-story test files), taking gate 1 from 82/113 to **86/113 (76.1%)** with
**no regression on any story**. Gate 2 is unchanged at **16 false positives** and is now the single
biggest commercial blocker; work it with the same held-out harness (B-094 no longer corrupts
eval-007/008). Open from B-096: the **per-test `pytest.skip` granularity** (one unresolved
placeholder still skips every resolved step).
