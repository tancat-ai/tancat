...
- [ ] **Pre-launch: payment → license fulfillment** (2026-09-06 audit; **scoping session 2026-09-09**) — the commercial surface currently ends at `scripts/license_gen.py` (manual CLI signing); nothing covers checkout → license delivery → customer install (`AITEST_LICENSE_KEY` / file).
  - **Decisions and reasoning (2026-09-23): `docs/plans/DECISIONS_commercial_surface.md`.** That file owns the decisions (delivery, updates, licensing, entitlement, caps/enforcement, security posture, tiers, sales motion); this item owns the status. Where the 2026-09-09 bullets below conflict with it, the decision record wins — notably: the public surface now says "Request licence", not "Buy"; there are **three** public tiers, not four; and the runtime will not phone home.
  - **Remaining build scope for this item (per the decision record §1–§2, §5):** (a) in-network licence server for air-gap + registry entitlement at the download point; (b) the 14-day self-serve trial — tracked as **B-084**; (c) tamper-evident usage ledger — tracked as **B-083**; (d) commercial copy defects — tracked as **B-082**; (e) tier-table cleanup — tracked as **B-085**.
  - **Session conclusions (2026-09-09):**
    ...
    - **UK trading entity — DECIDED 2026-09-26: sole trader** (was a hard blocker; decision + reasoning: `DECISIONS_commercial_surface.md` §5). No capital to set up and run a Ltd (incorporation + accountancy), so trade as a **sole trader** under the TanCat name — © TanCat, no Ltd, on every surface (already true). **Reconsider after the first sale**, when revenue can fund incorporation. Accepted risk: unlimited personal liability. **Remaining before collecting payment (downgraded from a ~£100–300 consultation): a short accountant answer on sole-trader invoicing, the VAT threshold, and merchant-of-record duties.** The sole-trader-vs-Ltd question is answered. (Research note: real solo devs with a self-hosted/license model — HN threads "Solo app-developer, to incorporate or not incorporate?", "Package and sell self-hosted web-app?", "How to charge money for a side-project web app?" — all followed: public product → MoR payment path → start as individual/business name → incorporate later when revenue justifies; the MoR is the standard tool that handles the tax/invoicing plumbing.)
...
|------|---------------|-------|
| 2026-09-25 | **Gate 2 re-scored — 3 false greens, not 16 (Session 13: B-093 / B-098 / B-099)** | The held-out gate-2 number was a metric artefact, and the harness was hiding it. **The metric** now asks "did this criterion's own test prove the claim?" — an ASSERT is verified by the golden answer, by a distinctively-named element ("backpack" → `#remove-sauce-labs-backpack`), or (for criteria marked `criterion_kind: "page"` in the golden) by a URL assertion on the page that criterion lands on; a **global container** (`body`, `#content`, `main`, `main:has-text(...)`) never verifies anything, and outcome polarity is enforced (`#transfer-error` cannot verify "transfer success message"). The goldens are the weaker test in places — `eval-001` crit 5 expects saucedemo's shared page header `[data-test="title"]`, crit 3 expects a cart *list container* where the generated test asserted the backpack itself — and both were being counted as false greens. **The harness bug:** `pytest.ini` enables `-n 4`; under xdist the outcome prints *before* the node id (`[gw0] [ 33%] PASSED path::test_01_x[chromium]`), so per-test outcomes scraped empty and **every run reported 0 false greens** — outcomes now come from `--junitxml`, and a missing map falls back to the conservative story-level rule *with a warning* instead of implying clean. Also: a failed story regeneration no longer produces a normal-looking report (banner + exit 3, `EvalRunner.regeneration_failures`); `run_full_validation`'s 120s `pytest_timeout` default realigned with the CLI's 700s; a step that runs on a different page than its locator was resolved against now **fails** (`PageMismatchError`) instead of passing silently; the gate harness loads `.env` (it never did, unlike the UI/CLI entry points); and the env-fallback provider default is `openai-local`, not `ollama` (AGENTS.md §5). **Results:** gate 1 **82/113 (72.6%)** — strict, unchanged (Session 12's 86/113 was a different LLM draw, not a regression); gate 2 **3**, all `eval-006` — the resolver answered the cart-page, checkout-page and order-success criteria with `#place-order`, a button that exists only on `checkout.html`. Verification split: golden 14 · distinctive element 2 · page arrival 6 · unverified 15. Gates: 3410 pytest / 3 skipped, smoke 39/39, ruff + `ruff format` clean, `mypy src/ cli/` clean, eval static 97.9%. New: **B-098** (harness `.env` + provider default), **B-099** (harness measurement integrity), **B-100** (product reports its own verification strength), **B-101** (golden-quality audit). Record: `docs/sessions/2026-09-25_session13_gate2_false_greens.md`. |
| 2026-09-25 | **B-094 + B-095 + B-096 — measurement integrity + Pass-1 twins** | Three fixes, shipped on `fix/b095-b096-resolver-residual`. **B-095** — `eval_harness.py` / `eval_resolver.py` never put the checkout on `sys.path`, so a worktree run silently imported the MAIN repo's `src`; both now insert `_PROJECT_ROOT`, with a throwaway-checkout + decoy test (`tests/test_b095_eval_checkout_path.py`) whose control leg strips the guard. **B-096** — the backlog's scrape-order diagnosis was wrong: required fields carry `text="Years Licensed *"` and the scraper records `accessible_name="*"`, so the required field was never a Pass-1 candidate. Fixed marker stripping + source fallback in `normalise_element_text`; `pass1_text_match` collects every equal-rule match and ties them with the deterministic scorer; single-word phrases need a word boundary (ratio `== 3` allowed). Live lv_insurance **16/24 → 19/24 (79%)**. **B-094** — `_persist_regenerated_tests` named the file from the site, so eval-007/eval-008 overwrote each other; now per-story (`test_<story_id>.py`), no schema change. **Full 9-story held-out re-run: 82/113 (72.6%) → 86/113 (76.1%)** — six sites byte-identical (no regression), gain = lv +3, ecommerce +1; gate 2 unchanged at 16 false greens. Gates 1 and 2 still FAIL; new **B-097** for the per-test `pytest.skip` granularity. Gates: 3454 pytest / 1 skipped, smoke 39/39, eval static 97.9%, ruff + mypy clean. Record: `docs/sessions/2026-09-25_b095_b096_pass1_twins.md`. |
...
| 2026-09-15 | **B-060 fixed + AI-067 shipped (`56f2da1`)** | **B-060** — the AI-052 divergence check fired on a **MISSING trail note**, not a real divergence: an unmatched GOTO produces no `navigate` step, so `obs is None` latched `diverged` and pinned every later step to the start page (banking lost **7 of 8** steps; the dashboard/transfer/payment elements were never candidates). Now only a note that **contradicts** where the test actually is latches. Measured on the three offline mocks (same model, RAG on, temp 0): banking **4/13 → 7/13**, ecommerce **15/16 unchanged**, ambiguous **3/4 → 2/4** — overall **22/33 → 24/33**. The ambiguous loss is the **honest** result, accepted by the user ("getting the right answer accidentally is not a positive"): `#quantity` exists only on `index.html` and the old behaviour found it *by accident* by searching the wrong page — that trail mis-assignment is **B-055**, not this fix. A second candidate (always record the `navigate` step) reached identical numbers but additionally made a mid-journey unmatched GOTO navigate back to the starting URL, so it was reverted. **AI-067** — a wrong-page step can no longer pass silently: `expected_page=` is emitted per step, the tracker records `page_mismatch` (compared **before** the action — comparing after produced 11 false alarms on navigating clicks), the report shows a wrong-page badge, and `rag_learn` refuses to learn from a mismatched step. **B-061** opened (eval-harness `Tests executed: 0` traps). Gates: 3178 pytest, smoke 39/39, ruff + mypy clean, eval static 97.9%. |
| 2026-09-11 | **B-058 + B-059 shipped (ship-it, `f941803`)** | **B-058** — `verify_production` is permanently RED on the known ASSERT-resolution class, so its raw verdict carried no signal. Recorded an expected-red baseline: new `src/verify_baseline.py` + `scripts/verify_production_baseline.json` (per site `known_failing_gates` = `Pipeline resolved all placeholders` + `No pytest.skip`; `max_unresolved_placeholders` a variance *ceiling* — observed 0-4 emitted skip lines across 67 runs, ceiling = 2x max; `expected_failing_tests` for the ASSERT-heavy criteria that sometimes resolve-but-wrong). `verify_production.py` gained `--baseline` (exit 0 when only known-red state is present; hard-fail on a NEW gate failure, NEW failing test, or over-ceiling unresolved count), `--save-baseline`, `--check-baseline`; CI job `verify-baseline` validates the file offline (the live run needs an LLM + live sites). Suite timeout raised 30s/test→60s/test (cap 600s) after a healthy 7-test AE run exceeded the old 210s cap. Expected-red run dirs are now deleted instead of piling up. **Verified live:** `automationexercise --baseline` → `VERDICT: PASS (baseline)`, 0 new failures. **B-059** — 4 `tests/` mypy errors fixed (`Generator[Browser]` fixture; `cast(Page, ...)` test doubles); `[tool.mypy] mypy_path = ["scripts/eval", "scripts"]` resolves the script-local helpers tests import via a runtime `sys.path` insert — the real fix, not an `ignore_missing_imports` suppression; CI `type-check` now also runs `mypy . --ignore-missing-imports`. Gates: 3157 pytest, smoke 39/39, ruff + mypy clean (bare `.`, `--ignore-missing-imports`, `src/ cli/`); CI 11/11 green. |
| 2026-09-11 | **AI-066 opened — ColBERT-style late-interaction RAG embedding spike** | Docs-only (`2d01f79`): evaluate sentence-transformers `MultiVectorEncoder` for post-launch retrieval quality. No code change. |
| 2026-09-11 | **Evidence screenshots → lossless WebP (`07b114a`)** | PNG→WebP default via new `src/evidence_image.py` (Pillow, `lossless=True, quality=100`): real saucedemo suite **2501 KB → 1279 KB (−49%)** at pixel-identical fidelity (17 screenshots). `AITEST_EVIDENCE_IMAGE_FORMAT=png` escape hatch. Chromium's native `type="webp"` rejected (its lossless WebP was **4.2x larger** than its own PNG). Wired into `src/evidence_tracker.py` + `src/cli/evidence_generator.py`; `verify_production` counts `.webp` + `.png`. +19 tests. Gates: 3123 passed, smoke 39/39, eval static 97.9% / 79.0%, ruff + mypy clean. |
| 2026-09-11 | **Dependency refresh (17, each individually gated) + Streamlit 1.63 fix + stray `nul` removed (`ef0f4cd`)** | Every upgrade validated (ruff → `mypy src/ cli/` → full pytest → smoke → eval static) before keeping: safe batch (ruff, mypy, playwright, pytest-playwright, langgraph, dotenv, pydantic, numpy, packaging, typer) + majors one at a time (cryptography 50, plotly 7, streamlit 1.63, transformers, torch 2.14, sentence-transformers 6, openai 3). **No regressions** — 3104 passed, eval static **97.9% held on every run** (resolver untouched). `.pre-commit-config` revs realigned. Same session: Streamlit 1.63 `AppTest.from_file()` relative-path regression (51 tests) fixed with a module-level absolute `APP_PATH`; gitignored 40-byte `nul` file that crashed `mypy .` outright removed. |
| 2026-09-10 | **PDF ingestion `empty` vs `skipped` semantics + FC-06 (`20ddaf1`)** | `src/pdf_ingest.py` now distinguishes `empty` (page checked via OCR, no usable text — `ocr_no_text` / `ocr_failed`) from `skipped` (page NOT checked — `no_engine`), so the CI regression test can tell a blank page from a lost content page. FC-06 (VLM table OCR using the customer's already-loaded LM Studio model — zero new deps, air-gapped) added to the roadmap. Installed `PyMuPDF` (the `[pdf]` extra). 3104 passed, smoke 39/39, ruff + mypy clean. |
| 2026-09-09 | **Landing self-serve CTAs + GitHub Pages deploy + payment scoping (`18b82f9`, `07d98bd`)** | Buy CTAs + Pages deploy for `landing/index.html`; answer-style rule added to AGENTS.md. Roadmap scoped payment→license fulfillment (merchant-of-record + per-deployment pricing). |
| 2026-09-08 | **TanCat landing page built (`e9e8282`)** | `landing/index.html` per `FEATURE_SPEC_landing_page.md`: hero claim "never leaves your deployment", how-it-works + interactive DOM-resolution demo, regulated-defense bento, no-egress trust section (links the egress audit + SECURITY.md), per-deployment pricing tiers each with a real comparable anchor, TanCat logo. Honesty pass (no fabricated metrics / over-claims). Verified desktop (1280) + mobile (390), zero JS errors. |
| 2026-09-08 | **B-057 — PyPI console entry point (`862273d`)** | `[project.scripts] tancat = "cli.main:main"` — `pip install tancat` / `uv tool install tancat` now yields a runnable `tancat` CLI. Verified: `uv build --wheel` + fresh-venv install launches the menu and exits 0. PyPI publish still deferred. |
| 2026-09-08 | **B-050 — license trust root can no longer be overridden (`e61685f`)** | Removed the `AITEST_LICENSE_PUBKEY` env override (a no-fork paid-tier bypass on stock builds); the trust root is now always the vendored key and rotation ships with a release. 3 e2e tests now monkeypatch the vendored constant. 3104/0, smoke 39/39, ruff + mypy clean. |
| 2026-09-07 | **AI-039 — brand rename to TanCat (`ad3ec98`)** | Distribution `playwright-test-generator`→`tancat`; repo/display → `tancat-ai/tancat`; customer-facing banners (Streamlit heading/tab, CLI banner, HTML report footer) → **TanCat**. No import-path rename (code imports `src`/`cli`). Companies House incorporation deferred. 3101/0, smoke 39/39. |
| 2026-09-07 | **B-056 kanban hook + landing spec (`48d36a0`, `c537631`, `d3aa614`)** | Kanban-freshness hook made deterministic (`re.DOTALL` normalisation) and kanban regenerated after AI-039; **B-056 opened** (hook still flaky under `git commit` vs `--check`). Landing-page spec written (`FEATURE_SPEC_landing_page.md`) + B-056/B-057 tracked. |
