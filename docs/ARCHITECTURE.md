...
| `src/link_scoping.py` | B-088 link-criterion scoping: `is_link_criterion()`, `is_anchor()`, `link_name_matches()`, `scope_pages_to_links()`. Narrows a link-resolution criterion to anchors and rejects a pick that does not match the named link (honest skip, never a lookalike). Used by `element_matcher.py`. |
| `src/content_scoping.py` | B-092 image/heading criterion scoping: `is_image_criterion()`, `is_heading_criterion()`, `is_image_element()`, `is_heading_element()`, `kind_matches()`, `scope_pages_to_images()`, `scope_pages_to_headings()`. Narrows an image criterion to `<img>` and a headline criterion to `h1`; the post-resolution `kind_matches()` guard rejects the wrong element kind. Used by `element_matcher.py` and `placeholder_scorers.py`. |
